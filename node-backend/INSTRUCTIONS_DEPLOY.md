...
He generado un archivo llamado `database_complete.sql` dentro de la carpeta `node-backend`. Este archivo contiene **toda** la estructura actual y los datos de tu sitio.
...
3. Selecciona la pestaña **Importar** y sube el archivo `node-backend/database_complete.sql`.
