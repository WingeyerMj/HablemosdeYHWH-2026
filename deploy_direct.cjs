...
try {
  ...
  const tarCmd = `tar --exclude=node_modules --exclude=vendor --exclude=.git --exclude=.env --exclude=*.zip --exclude=temp_zip_check --exclude=video-output --exclude=node-backend/node_modules --exclude=node-backend/.env --exclude=node-backend/public/assets/parashot --exclude=node-backend/public/uploads --exclude=public/uploads -czf "${LOCAL_TAR_PATH}" .`;
